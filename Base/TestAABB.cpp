...
void TestAABB::SmokeTest() {
    AABB2 test;
    TASSERT(test.getCenter() == (Vector2(0, 0)));
    TASSERT(test.getRadius() == (Vector2(0, 0)));

    AABB2 test2(Vector2(10, -10), Vector2(0, 50));
    TASSERT(test2.getCenter() == (Vector2(10, -10)));
    TASSERT(test2.getRadius() == (Vector2(0, 50)));

    AABB2 test3(Vector2(10, 10), Vector2(-10, 10));
    TASSERT(test3.getCenter() == (Vector2(10, 10)));
    TASSERT(test3.getRadius() == (Vector2(10, 10)));
}

void TestAABB::TestRadAndDia() {
    AABB2 test;
    TASSERT(test.getRadius() == (Vector2(0, 0)));
    TASSERT(test.getRadius()[0] == 0);
    TASSERT(test.getRadius()[1] == 0);
    TASSERT(test.getDiameter()[0] == 0);
    TASSERT(test.getDiameter()[1] == 0);

    AABB2 test2(Vector2(10, -10), Vector2(-5, 50));
    TASSERT(test2.getRadius() == (Vector2(5, 50)));
    TASSERT(test2.getRadius()[0] == 5);
    TASSERT(test2.getRadius()[1] == 50);
    TASSERT(test2.getDiameter() == (Vector2(10, 100)));
    TASSERT(test2.getDiameter()[0] == 10);
    TASSERT(test2.getDiameter()[1] == 100);
    
}

void TestAABB::TestMin() {
    AABB2 test(Vector2(10, -10), Vector2(5, 50));
    TASSERT(test.getMin() == (Vector2(5, -60)));
    TASSERT(test.getMin()[0] == 5);
    TASSERT(test.getMin()[1] == -60);
}

void TestAABB::TestMax() {
    AABB2 test(Vector2(10, -10), Vector2(5, 50));
    TASSERT(test.getMax() == (Vector2(15, 40)));
    TASSERT(test.getMax()[0] == 15);
    TASSERT(test.getMax()[1] == 40);
}
...
void TestAABB::TestTouching() {
    ...
    TASSERT( test1.overlapping(test2));
    TASSERT( test1.overlapping(test3));
    TASSERT( test1.overlapping(test4));
    TASSERT(!test2.overlapping(test3));
    TASSERT(!test2.overlapping(test4));
    TASSERT( test3.overlapping(test4));
}
...
void TestAABB::TestSweep() {
    ...
    TASSERT(AABB2::Sweep(box, Vector2(10, 0), wall, Vector2(0, 0), r0, r1));
    TASSERT_EQ(r0, .8f); TASSERT_EQ(r1, 1);
...
void TestAABB::TestSweepLine() {
    ...
    TASSERT_EQ(r0, .4f); TASSERT_EQ(r1, .6f);
}
