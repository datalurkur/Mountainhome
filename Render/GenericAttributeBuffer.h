...
#ifndef _GENERICATTRIBUTEBUFFER_H_
#define _GENERICATTRIBUTEBUFFER_H_
#include "Buffer.h"
 ...
 *  GenericAttributeBuffers represent a single vertex. For example, a VertexArray may have
 *  an GenericAttributeBuffer for vertex position, vertex normal, and vertex texture
 *  coordinates. All of these combines form a single vertex, which is used by the graphics
 *  hardware to render something to screen. GenericAttributeBuffer can be enabled and
 *  disabled individually, but it is probably easiest to work with VertexArrays. Remember,
 *  when binding to channels, the channels need to be associated with attributes in the
 *  active Shader.
 ...
 * \seealso VertexArray
 * \seealso Shader */
class GenericAttributeBuffer : public Buffer {
public:
    /*! Creates a new GenericAttributeBuffer
