...
PathNode *AStarNode::getPathNode() { return _pathNode; }
...
int AStarPathFinder::getPath(Vector3 start, Vector3 end, Path &path) {
    ...
    openList.push_back(new AStarNode(NULL, NODE(end[0], end[1], end[2]), 0, distance(start, end)));
    ...
    while(!openList.empty()) {
        ...
        updateOpenList(currentNode, openList);
...
void AStarPathFinder::updateOpenList(AStarNode *currentNode, AStarList &openList) {
    ...
    for(ConstEdgeIterator itr = edgeList.begin(); itr != edgeList.end(); itr++) {
        ...
        if(itr == openList.end()) {
            openList.push_back(new AStarNode(currentNode, thisPathNode, thisWeight, distance(thisPathNode->getCenter(), start)));
        }
    }
}
